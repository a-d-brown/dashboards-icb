...
DB_PATH = "prescribing.duckdb"  # adjust path if relevant

# Map the dataset_type (the friendly names in your selectbox) to a filter in the DB. You will need to populate this mapping with the actual BNF codes
# or presentation identifiers used in DB. Example below is placeholder.
DATASET_TO_PRESENTATION_FILTER = {
    "Antibacterials": "0501%",    # match BNF codes that start with 0501
    # ... fill for all dataset_measures keys
    # If datasets are identified by BNF Chemical Substance strings, use those values here
}
...
def load_data_from_db(dataset_type, months_of_history=24):
    """
    Return (icb_df, national_df_or_None)
    - icb_df: practice-level rows for the selected dataset
    - national_df: aggregated national-level rows by Month (sum of all practices for same filter)
    """
    import time
    con = duckdb.connect(DB_PATH, read_only=True)

    # get pattern
    presentation_pattern = DATASET_TO_PRESENTATION_FILTER.get(dataset_type)

    # 1) get latest date in the date table to compute cutoff
    t0 = time.time()
    try:
        max_date_row = con.execute('SELECT MAX(date) AS max_date FROM "date"').fetchdf()
        if not max_date_row.empty and pd.notna(max_date_row.loc[0, "max_date"]):
            latest_date = pd.to_datetime(max_date_row.loc[0, "max_date"])
        else:
            # fallback to today if table weirdly empty
            latest_date = pd.Timestamp.today()
    except Exception as e:
        # fall back - don't fail the whole app
        latest_date = pd.Timestamp.today()
        st.warning(f"Couldn't read latest date from DB, falling back to today: {e}")

    cutoff = (latest_date - pd.DateOffset(months=months_of_history)).replace(day=1)
    cutoff_str = cutoff.strftime("%Y-%m-%d")

    t1 = time.time()
    st.write(f"DB latest date: {latest_date.date()}  → using cutoff: {cutoff_str}  (metadata query {t1-t0:.2f}s)")

    # Build WHERE date filter clause (used in both queries)
    date_filter_clause = f"d.date >= DATE '{cutoff_str}'"

    # Build presentation filter (if present)
    presentation_where = ""
    if presentation_pattern and presentation_pattern != "NO_PATTERN_YET":
        presentation_where = f"pres.bnf_code LIKE '{presentation_pattern}'"

    # Combine WHERE parts carefully
    def combine_where(*clauses):
    ...
    # ----------------------
    # Practice-level query (only select necessary columns)
    # ----------------------
    base_query = f"""
    SELECT
        pn.presentation_id AS presentation_id,
        pres.bnf_code AS "BNF Presentation plus Code",
        NULL::VARCHAR AS "BNF Chemical Substance plus Code",
        pn.date_id AS date_id,
        d.date AS "Year Month",
        pn.practice_id AS practice_id,
        pr.code AS "Commissioner / Provider Code",
        pr.code AS Practice,
        COALESCE(pn.items, 0) AS Items,
        COALESCE(pn.net_cost, 0) AS "Actual Cost",
        0 AS "ADQ Usage",
        0 AS "DDD Usage",
        COALESCE(ls.total, NULL) AS "List Size",
        COALESCE(lsn.total, NULL) AS "COPD List Size"
    FROM prescribing_norm pn
    LEFT JOIN presentation pres ON pres.id = pn.presentation_id
    LEFT JOIN practice pr ON pr.id = pn.practice_id
    LEFT JOIN "date" d ON d.id = pn.date_id
    LEFT JOIN list_size ls ON (ls.practice_id = pn.practice_id AND ls.date_id = pn.date_id)
    LEFT JOIN list_size_norm lsn ON (lsn.practice_id = pn.practice_id AND lsn.date_id = pn.date_id)
    {combine_where(presentation_where, date_filter_clause)}
    """

    t2 = time.time()
    st.write(f"Executing practice-level query...")

    try:
        icb_df = con.execute(base_query).fetchdf()
        t3 = time.time()
        st.write(f"Practice-level rows: {len(icb_df):,}  (query {t3-t2:.2f}s)")
    except Exception as e:
        st.error(f"Practice-level query failed: {e}")
        icb_df = pd.DataFrame()
        t3 = time.time()

    # ----------------------
    # National aggregation (do aggregation in SQL, date-filtered)
    # ----------------------
    national_sql = f"""
    SELECT
        pn.date_id,
        d.date AS "Year Month",
        SUM(COALESCE(pn.items, 0)) AS Items,
        SUM(COALESCE(pn.net_cost, 0)) AS "Actual Cost",
        0 AS "ADQ Usage",
        0 AS "DDD Usage",
        SUM(COALESCE(ls.total, 0)) AS "List Size",
        SUM(COALESCE(lsn.total, 0)) AS "COPD List Size"
    FROM prescribing_norm pn
    LEFT JOIN list_size ls ON (ls.practice_id = pn.practice_id AND ls.date_id = pn.date_id)
    LEFT JOIN list_size_norm lsn ON (lsn.practice_id = pn.practice_id AND lsn.date_id = pn.date_id)
    LEFT JOIN presentation pres ON pres.id = pn.presentation_id
    LEFT JOIN "date" d ON d.id = pn.date_id
    {combine_where(presentation_where, date_filter_clause)}
    GROUP BY pn.date_id, d.date
    ORDER BY d.date;
    """

    t4 = time.time()
    st.write("Executing national aggregation query...")
    try:
        national_df = con.execute(national_sql).fetchdf()
        t5 = time.time()
        st.write(f"National rows: {len(national_df):,}  (query {t5-t4:.2f}s)")
    except Exception as e:
        st.warning(f"National aggregation failed: {e}")
        national_df = None
        t5 = time.time()

    con.close()
    return icb_df, national_df
...
## ── Data Loading ───────────────────────────────
# Load raw frames from DB
icb_raw_df, national_raw_df = load_data_from_db(dataset_type)

# If dataset is High Cost Drugs, your old behavior kept icb preprocessed = raw
# (you might still need special handling for HCD) — preserve that behavior:
if dataset_type == "High Cost Drugs":
    # The CSV loader previously used the raw CSV as preprocessed for HCD
    icb_data_preprocessed = icb_raw_df.copy()
    national_data_preprocessed = None
else:
    # Run the same preprocess function you use for CSVs.
    # The function expects Year Month column and returns date, period_tag etc.
    try:
        icb_data_preprocessed = preprocess_prescribing_data(
            icb_raw_df.copy(), is_national=False, mapping=sicbl_legend_mapping
        )
    except Exception as e:
        # Helpful error if expected columns are missing
        st.error(f"Error preprocessing ICB data from DB: {e}")
        raise

    # National frame: may be None if aggregation failed; otherwise preprocess as national
    if national_raw_df is not None and not national_raw_df.empty:
        try:
            national_data_preprocessed = preprocess_prescribing_data(
                national_raw_df.copy(), is_national=True
            )
        except Exception as e:
            st.warning(f"Could not preprocess national data: {e}")
            national_data_preprocessed = None
    else:
        national_data_preprocessed = None
